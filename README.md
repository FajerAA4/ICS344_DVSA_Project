...
Same root cause as Lesson 1, viewed from the dependency-management angle. `npm audit` flagged `node-serialize` as critical-severity  with no fix available. The library must be removed entirely — there is no safe version to upgrade to. Fix: replace all `serialize.unserialize()` calls with `JSON.parse()` and run `npm uninstall node-serialize`. 
